...
public:
    ...
    virtual bool IsObjectVisible(BaseObject* baseObject) = 0;

    // Gets new geometry and instances on the base object. Also flags instances on the main object for deletion.
    virtual std::vector<std::pair<IGeometryGenerationData*, std::vector<Instance*>>> GetNewGeometryAndInstances(BaseObject* baseObject, std::unordered_map<long, long, Geometry*>* existingGeometreis) = 0;
};
