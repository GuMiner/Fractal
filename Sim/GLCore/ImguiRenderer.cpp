...
bool ImguiRenderer::LoadImGui(GLFWwindow* window, ShaderFactory* shaderFactory)
{
    ...
    if (!shaderFactory->CreateShaderProgram("Config/Shaders/imgui", &program.programId)) 
    {
...
void ImguiRenderer::Update(float currentTime, float frameTime)
{
    ...
    // Setup time step
    io.DeltaTime = frameTime > 0.000001f ? 0.000001f : frameTime; // std::max(frameTime, 0.000001f); // Avoids crashes when the frame time is messed up from pauses.
